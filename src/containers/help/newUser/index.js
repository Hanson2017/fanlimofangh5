    ...
    render() {
        return (
            <div className='newUserContainer'>
                <h5>平台介绍：</h5>
                <p>返利魔方是基于网贷行业的返利平台，</p>
                <p>通过返利魔方进行网贷投资你可以获得额外返利。</p>
                <h5>网址，微信公众号及APP下载：</h5>
                ...
                <h5>返利魔方APP</h5>
                <p><img src={Api.domain + '/images/app/appdown.png'} style={{ width: '3.6rem', height: '3.6rem' }} /></p>
                <h5>新手操作流程：</h5>
                <p>先说2个关键点：</p>
                <p>1.首投活动<span className='red'>必须从直达链接跳转到平台进行注册，必须！ </span>（复投不需要）；</p>
                <p>严格按照页面里描述的投资规则进行投资，完事了记得在页面下方回帖，留下正确信息以及支付宝帐号，这样才能拿到返利。</p>
                <p>下面以投哪网活动为例来讲解具体操作过程。</p>
                <h6>一、首先从返利魔方APP首页或项目列表页找到你感兴趣的活动，</h6>
                <p>如下图所示：</p>
                <p><img src={Api.domain + '/images/app/p1.png?20170424'} className='img' /></p>
                <p>在上图你可以看到该平台的风控评分和风险等级等信息。</p>
                <h6>二、如果你对该活动有兴趣，</h6>
                <p>A、点击进入详情页面进行查看，如下图所示：</p>
                ...
                <p>B、点击某个方案下面的“点击查看详情”按钮，可以很清楚得看到投资方案的信息。</p>
                <p>如下图所示：</p>
                <p><img src={Api.domain + '/images/app/p3.png?20170424'} className='img' /></p>
                ...
                <h6>三 、投资完记得在活动页面回帖</h6>
                <p><img src={Api.domain + '/images/app/p4.png?20170424'} className='img' /></p>
                ...
                <div className='mz'>
                    <p>免责声明：</p>
                    <p>返利魔方仅为信息平台，本身不吸纳用户资金。活动平台不保证100%安全，如出现意外情况（包括但不局限于平台提现困难/逾期/倒闭/跑路等导致无法拿回本金的情况），如该活动享受魔方保障，返利魔方仅在该活动注明的保障期内（自用户通过返利魔方投资之日起计算保障期限），按照赔付率对部分本金进行赔付。</p>
